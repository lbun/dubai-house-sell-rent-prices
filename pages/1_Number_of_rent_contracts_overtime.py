...
def plotting_num_transactions():
    ...
    df = duckCon.execute("SELECT * FROM main_golden.monthly_rent").fetch_df()
    ...
    group_area_1y = df[df["year_month"].isin(months_list)].groupby(["year_month", "year"]).count().reset_index()
    # group_area_1y["year_month"] = pd.to_datetime(group_area_1y["year_month"].astype(str))

    # Create an interactive line plot using plotly express
    fig1 = px.line(group_area_1y,
                x='year_month', y='year', markers=True, #color='type',
    ...
    group_area = df.groupby(["year"])["year_month"].count().reset_index()
    group_area["year"] = pd.to_datetime(group_area["year"].astype(str))
    group_area.head(2)

    # Create an interactive line plot using plotly express
    fig2 = px.line(group_area,
                x='year', y='year_month', markers=True, #color='type', 
